...
K0_LIST = tuple(np.geomspace(1.0e-12, 2.5e-12, 2))
...
ALPHA_LIST = (0.40, 0.55, 0.70, 0.85, 1.00)
...
# Differential rotation, j-constant law:  Omega(varpi) = Om_c A^2/(A^2+varpi^2)
# with A given as a fraction of the background equatorial radius. A -> infinity
# is uniform rotation; small A concentrates the shear in the core. The
# centrifugal potential is then closed form,
#     Psi(varpi) = -(Om_c^2 A^2/2) varpi^2/(A^2+varpi^2),
# so Omega = Omega(varpi) keeps the centrifugal force conservative and the
# whole non-barotropic machinery goes through unchanged with Phi replaced by
# Phi_eff = Phi + Psi. Gravity itself is still solved from rho alone.
ROT_A_LIST = (0.5, 1.0, 2.0)
# rotational support as T/|W|. Uniformly rotating white dwarfs shed mass near
# T/|W| ~ 0.09; differential rotation goes further, which is the whole reason
# the ultramassive literature uses it.
ROT_LIST = (0.0, 0.02, 0.04, 0.06, 0.08)
# Mass shedding: reject when the centrifugal force at the equatorial surface
# reaches gravity there, since past that point the "star" is unbound.
SHED_GATE = 0.95

OUT = HERE / "nonbarotropic_rotating_scan.csv"


def solve_one(args):
    k0, alpha, rho_c, tor_w, rot_w, a_rot = args
    try:
        ...
        def toroidal(rho_now, Phi_now):
            ...
            S = nonlocal_src + beta * dbeta
            g = np.where(varpi > 0,
                         -S / (4 * np.pi * np.maximum(varpi, 1e-30)**2), 0.0)
            return Bphi, g

        R_eq0 = diag.equatorial_polar_radii(H0, r, th)[0]
        A_len = a_rot * R_eq0
        rot_shape = varpi**2 / (A_len**2 + varpi**2)        # Psi / (-Om_c^2 A^2/2)
        j_shape = (A_len**4) * varpi**2 / (A_len**2 + varpi**2)**2   # Omega^2 varpi^2 / Om_c^2

        def rotation(rho_now, Phi_now):
            """Centrifugal potential normalised to the target T/|W|."""
            if rot_w <= 0.0:
                return np.zeros_like(rho_now), 0.0, 0.0
            T_unit = 0.5 * diag.volume_integral(rho_now * j_shape, r, th)
            W_now = abs(diag.gravitational_energy(rho_now, Phi_now, r, th))
            if T_unit <= 0:
                return np.zeros_like(rho_now), 0.0, 0.0
            om_c2 = rot_w * W_now / T_unit
            Psi = -0.5 * om_c2 * A_len**2 * rot_shape
            return Psi, om_c2, om_c2 * T_unit

        Psi, om_c2, T_rot = rotation(rho0, Phi)
        ...
        for _ in range(N_OUTER):
            if SELF_CONSISTENT_U:
                # the poloidal source is rho M'(u) with M' = k0, so it must
                # follow the CURRENT density, not the field-free background
                src_new = -4.0 * np.pi * varpi**2 * rho * k0
                src = (1.0 - U_RELAX) * src + U_RELAX * src_new
                u = solve_gradshafranov(src, r, th, lmax=LMAX)
            Bphi, g = toroidal(rho, Phi)
            Psi, om_c2, T_rot = rotation(rho, Phi)
            Phi_eff = Phi + Psi
            P = integrate_pressure(P_axis, Phi_eff, g, u, r, th, seeds)
            P_r, _ = grads(P, r, th)
            Phi_r, _ = grads(Phi_eff, r, th)
            u_r, _ = grads(u, r, th)
            ...
            # Divergence guard. At T/|W| >= 0.08 with A = R_eq the iteration
            # runs away, and without this it returned M = 2e13 Msun with a
            # virial error of 1705 as though it were a measurement.
            if not np.isfinite(rho).all():
                return dict(k0=k0, alpha=alpha, rho_c=rho_c, tor_w=tor_w,
                            rot_w=rot_w, a_rot=a_rot, status="diverged")
            if units.g_to_msun(scf_mod.total_mass(rho, r, th)) > 6.0:
                return dict(k0=k0, alpha=alpha, rho_c=rho_c, tor_w=tor_w,
                            rot_w=rot_w, a_rot=a_rot, status="runaway")
        ...
        # Rotation enters the scalar virial as 2T.
        VE = abs(2.0 * T_rot + W_signed
                 + 3.0 * diag.volume_integral(P, r, th) + E_mag) / W

        # Mass shedding. At the equatorial surface, centrifugal against
        # gravity; at 1 the outermost matter is no longer bound and the
        # configuration is not a star.
        jeq = len(th) // 2
        kk = np.flatnonzero(rho[:, jeq] > 0)
        if kk.size and om_c2 > 0:
            R_eq = float(r[kk[-1]])
            om2_eq = om_c2 * A_len**4 / (A_len**2 + R_eq**2)**2
            gr_eq = abs(float(grads(Phi, r, th)[0][kk[-1], jeq]))
            shed = float(om2_eq * R_eq / max(gr_eq, 1e-30))
        else:
            shed = 0.0
        TW = T_rot / max(W, 1.0)
        ...
        if RETURN_FIELDS:
            return dict(rho=rho, P=P, Phi=Phi, Psi=Psi, u=u, g=g, Bphi=Bphi,
                        T_rot=T_rot, om_c2=om_c2, A_len=A_len, shed=shed,
        ...
        # T/|W| drifting off its target is the other divergence signature:
        # the runaway cases asked for 0.08 and 0.12 and delivered 0.119 and
        # 0.333, so a solution that misses by more than 5% is not the
        # configuration that was requested and is not reported as one.
        on_target = (rot_w == 0.0) or abs(TW - rot_w) <= 0.05 * rot_w

        ok = (physical and on_target and B_tot_max < B_C and ledoux < 0.30
              and VE < VE_GATE and f_core > 0.0 and shed < SHED_GATE)
        return dict(k0=k0, alpha=alpha, rho_c=rho_c,
                    ...
                    rot_w=rot_w, a_rot=a_rot, TW=TW, shed=shed,
                    on_target=on_target,
...
def main():
    ...
    grid = list(itertools.product(K0_LIST, ALPHA_LIST, RHO_C_LIST,
                                 TOR_W_LIST, ROT_LIST, ROT_A_LIST))
    grid = [a for a in grid if not (a[4] == 0.0 and a[5] != ROT_A_LIST[0])]
    ...
    for d in sorted(rows, key=lambda x: (x["rho_c"], x["k0"], x["alpha"],
                                         x.get("tor_w", 0), x.get("rot_w", 0),
                                         x.get("a_rot", 0))):
        head = (f"  {d['rho_c']:.0e}  {d['k0']:.2e} {d['alpha']:.2f} "
                f"{d.get('tor_w', 0):5.3f} {d.get('rot_w', 0):5.2f} "
                f"{d.get('a_rot', 0):4.1f}  {d['status']:6s}")
        if "mu_mean" in d:
            print(head + f" {d['M']:6.3f} {d['mu_p01']:.3f}-{d['mu_p99']:.3f} "
                         f"{d['f_bad']:6.4f} "
                         f"{d['EmW']:9.5f} {d['bt_bp']:6.2f} {d['shed']:5.2f} "
    ...
    with OUT.open("w") as f:
        ...
        f.write("k0,alpha,rho_c,status,M_msun,M0_msun,mu_mean,mu_lo,mu_hi,"
                ...
                "bt_bp,f_core,rot_w,a_rot,T_over_W,shed,strict,beats_ceiling,B_over_Bc\n")
        for d in rows:
            f.write(",".join(str(d.get(k, "")) for k in
                             ("k0", "alpha", "rho_c", "status", "M", "M0",
                              ...
                              "bt_bp", "f_core", "rot_w", "a_rot", "TW",
                              "shed", "strict", "beats_ceiling",
                              "B_over_Bc")) + "\n")
    print(f"\nwrote {OUT.name}")

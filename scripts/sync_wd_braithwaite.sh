#!/usr/bin/env bash
# Syncs the Braithwaite Castro problem between its live build location
# (castro/Exec/science/wd_braithwaite/ -- inside the gitignored castro/
# clone, where it actually compiles and runs) and its versioned mirror
# (castro_problems/wd_braithwaite/ -- tracked in this repo).
#
...
#
# Usage:
#   scripts/sync_wd_braithwaite.sh save     # live -> mirror (before committing)
#   scripts/sync_wd_braithwaite.sh restore  # mirror -> live (after a fresh clone/reset)

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LIVE="$REPO_ROOT/castro/Exec/science/wd_braithwaite"
MIRROR="$REPO_ROOT/castro_problems/wd_braithwaite"

...
$\Emag$ at the minimum (erg) & $2.54\times10^{46}$ & $6.35\times10^{47}$ & $\times 25$ apart \\
growth after it & $+0.0485$/s & $+0.0196$/s & \emph{coarse grid faster} \\
\quad as an $e$-folding & $21$\,s & $51$\,s & \\
minimum to end of run & $+114\%$ & $+21\%$ & \\
\bottomrule
\end{tabular}
...
\paragraph{What is not.} The levels differ by a factor of $25$ at the minimum,
and the growth rate by a factor of $2.5$ --- with the \emph{coarse} grid
growing faster. That is the same direction and roughly the same factor as the
decay rate before it, which fell from $0.282$ to $0.141$\,s$^{-1}$ under the
same refinement. A rate that halves when the mesh is refined is what a
mesh-set process looks like, so the regrowth may be as numerical as the decay
was. Two resolutions cannot separate a shear dynamo whose rate happens to
scale that way from a numerical effect that does.

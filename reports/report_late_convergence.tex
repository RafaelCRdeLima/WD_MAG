...
%======================================================================
\section{The residual field does not converge}
\label{sec:residual}

The one place where extending the baseline made the disagreement worse.

\begin{table}[h]
\centering\small
\begin{tabular}{lrrr}
\toprule
late-time field & $192^3$ & $256^3$ & ratio \\
\midrule
$\Emag$ plateau (erg) & $\simeq 3\times10^{46}$ & $7.0\times10^{47}$ & $\mathbf{25}$ \\
$\max|B|$ (G) & $\simeq 1\times10^{12}$ & $6\times10^{12}$ & $6$ \\
trend for $t > 45$\,s & --- & $\mathbf{+0.012}$\,s$^{-1}$ & \emph{rising} \\
\bottomrule
\end{tabular}
\caption{The $192^3$ values are from the single late instant processed with the
field diagnostics, $t = 38.5$\,s; the $256^3$ values are means over
$t = 54$--$58.3$\,s. The trend is fitted to $\ln\Emag$ over $t > 45$\,s, with
$\Etor$ and $\Epol$ both rising.}
\label{tab:residual}
\end{table}

This is consistent with what Report~I already found: with no explicit
resistivity, the decay of $\Etor$ \emph{is} the numerical resistivity, and it
scales as $\mathrm{d}x^{2.4}$. Less numerical dissipation leaves more field. But
the consequences are sharper than the factor-of-two disagreement seen at
$t = 12$\,s:

\begin{itemize}\itemsep2pt
\item The ``disordered residual of order $10^{12}$\,G'' reported at $192^3$
      describes a floor set by the mesh, not a state of the star.
\item The factor $2170$ by which $\Emag$ falls at $192^3$ is an \textbf{upper
      bound} on the destruction, and not a tight one.
\item The $256^3$ run has not reached a final state at all. A run that is
      gaining magnetic energy when it stops has not finished.
\end{itemize}

\paragraph{Two readings, and two resolutions cannot separate them.} The
mechanism for genuine regeneration is available and it is not exotic: the
differential rotation survives and is still steepening, so there is free shear
energy, and winding residual poloidal field into toroidal is the most ordinary
thing a shear flow can do to a magnetic field. On that reading the finer grid
is showing the beginning of a shear dynamo that the coarse grid dissipates
before it can start.

The other reading is that both runs are on the same decaying track and the
finer one is simply further from the mesh floor, with the apparent rise being
the last decade of a decay it cannot resolve, contaminated by the pulsation.

The two look identical in the data we have. Distinguishing them needs either a
third resolution --- $384^3$, about five times the cost of $256^3$ --- or,
better, explicit resistivity, which would give the problem a physical
dissipation scale instead of letting the mesh set it. \textsc{Castro} does not

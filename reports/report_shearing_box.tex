...
$\alpha \sim 8\times10^{-4}$ and a braking time of order $130$\,s against a
run length of $78$\,s: \textbf{if the MRI ran at the level this box measures, it
should already have erased much of the differential rotation our runs find
steepening.} That estimate is an order of magnitude and is presented as one.
\end{abstract}
...
The braking time is comparable to the duration of the run. If the MRI operated
at the transport level this box measures, with the field the star actually has,
it would have erased a substantial fraction of the differential rotation within
the window we simulated. \textbf{Our runs instead show the profile steepening by
$22.6\%$.}

That is the tension stated quantitatively, and it is the reason the unresolved
MRI is not an academic caveat. It is also consistent in sign with
\citet{miravet2025}, who switch on a subgrid MRI closure in a differentially
rotating magnetised neutron star and find the inner profile flattening while we
find it steepening.

\paragraph{What would weaken this estimate.} It is a back-of-envelope and
should be read as one. $H \sim \Req$ is crude: the stress divergence acts over
whatever scale the field and rotation vary on, and a larger effective $H$ gives
a longer braking time. $W/B_0^2 = 32$ is measured at $\Pm = 16$ and $q = 1.5$,
not at our star's $\Pm$ or its range of $q$. The field used is the decayed
late-time one, and $\alpha \propto B_z^2$, so it is sensitive to exactly the
quantity Report~II shows is not converged. A factor of ten in the braking time
in either direction would not be surprising --- but a factor of $10^{4}$, which
is what ``the differential rotation is safe'' would require, would be.

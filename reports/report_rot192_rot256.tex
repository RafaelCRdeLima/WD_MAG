...
opposite direction. Both grids are now complete to $t = 12$\,s. Averaged over
the last two pulsation periods they agree on
$\Omega_{\mathrm{out}}/\Omega_{\mathrm{core}}$ to $1.4\%$ and on the peak field
to $7\%$: the survival of the differential rotation is a result and not a
resolution. What does not converge is the magnetic \emph{energy}, twice as
large on the finer grid at $t=12$\,s.
\end{abstract}
...
size, and above all the rotation, where the core, the outer shell and their
ratio all land within $3\%$ --- while everything about the \emph{energy} in the
field differs by a factor of two. The finer grid dissipates less, which is
exactly what an ideal-MHD run with no explicit resistivity should do.}
\label{tab:window}
\end{table}
...
\item \textbf{Only $192^3$ goes past $12$\,s.} The two grids now overlap fully
      to $t = 12$\,s, but the late evolution --- the settled $1.50$\,s
      pulsation, the halving of the braking rate once the field is gone, the
      steepening of the differential rotation to
      $\Omega_{\mathrm{out}}/\Omega_{\mathrm{core}} = 0.27$, and the
      $L_z = I\bar\Omega$ spin-up --- is measured at $192^3$ alone. Within the
      common window the two grids agree on the rotation to $1.4\%$, which is
      reassuring but is not a check on what happens after it.
...
$256^3$: complete, $t = 12$\,s ($7059$ steps).

...
$t = 12$\,s. Over $t = 12$ to $40$\,s, where both grids now exist, the star
loses angular momentum at $0.076\%$\,s$^{-1}$ against $0.081\%$\,s$^{-1}$ ---
converged to $6\%$ --- and \textbf{the profile steepens on both}, the fine grid
beginning some $6$\,s later and then steepening faster. $256^3$ has reached
$t = 40.5$\,s and is continuing towards $60$\,s.
\end{abstract}
...
$^{\ddagger}$\textbf{Amplitude not converged.} These two rows measure how the
star redistributes angular momentum internally. The $256^3$ run moves both the
same way, but by less over the same window ($-11.4\%$ and $-1.8\%$ against
$-15.8\%$ and $-5.9\%$ from $t = 12$ to $40$\,s), because its onset is some
$6$\,s later; from $t = 26$ onward it steepens faster than this one
(Sec.~\ref{sec:latewindow}). Read the direction, not the number.}
\label{tab:end}
\end{table}
...
\subsection{Past $12$\,s, and a lesson about baselines}
\label{sec:latewindow}

\label{sec:latewindow}

The $256^3$ run has been continued to $t = 40.5$\,s, which puts a second
resolution on most of the late evolution. Table~\ref{tab:late} compares the two
over a $28$\,s baseline, using means of two pulsation periods at each end so
the $\pm14\%$ breathing is averaged out rather than interpolated through.

\begin{table}[h]
\centering\small
\begin{tabular}{lrrl}
\toprule
$12$--$15$\,s $\to$ $37.5$--$40.6$\,s & $192^3$ & $256^3$ & verdict \\
\midrule
$M$ ($\Msun$) & $0.0\%$ & $+0.1\%$ & converged \\
$L_z$ & $-1.9\%$ & $-2.1\%$ & converged \\
\quad as a rate & $-0.076\%$/s & $-0.081\%$/s & converged to $6\%$ \\
$\Omega_{\mathrm{out}}$ & $-10.6\%$ & $-9.7\%$ & converged \\
\midrule
$\Omega_{\mathrm{out}}/\Omega_{\mathrm{core}}$ & $-15.8\%$ & $-11.4\%$ & same sign, onset delayed \\
$\Omega_{\mathrm{core}}$ & $+6.2\%$ & $+2.0\%$ & same sign \\
$L_z^{\mathrm{outer}}/L_z^{\mathrm{inner}}$ & $-5.9\%$ & $-1.8\%$ & same sign \\
\bottomrule
\end{tabular}
\caption{The two grids over $28$\,s. Both the global loss of angular momentum
and its rate agree to within a few percent, and so does the outer-shell
spin-down. The differential rotation steepens on both.}
\label{tab:late}
\end{table}

The gap in the lower block is a delay and not a suppression, which only a
window this long makes visible. Broken into segments:
...
\bottomrule
\end{tabular}
\caption{The finer grid starts about $6$\,s later --- it is still flat, even
rising slightly, while the coarse grid has already begun --- and then steepens
\emph{faster}. Over $t = 26$ to $40$\,s alone the ratio falls $8.2\%$ at
$192^3$ against $10.6\%$ at $256^3$. The smaller total in
Table~\ref{tab:late} is entirely the late start.}
\label{tab:segments}
\end{table}

This is worth stating plainly because an earlier version of this report drew
the opposite conclusion. Compared only out to $t = 26.5$\,s the $256^3$ ratio
had fallen $1.0\%$ against $8.2\%$ at $192^3$, and
$L_z^{\mathrm{outer}}/L_z^{\mathrm{inner}}$ appeared to move the other way, so
the steepening was recorded as a coarse-grid artefact. It was the baseline that
was too short: the fine grid had not yet started. Fourteen seconds was not
enough to separate a delayed onset from an absent effect, and twenty-eight is.

What can be said now: \textbf{the differential rotation steepens at both
resolutions}, at the same rate once both have begun, and the star loses angular
momentum at a rate the two grids agree on to $6\%$. What is still open is the
onset --- why the fine grid waits $6$\,s --- and whether the amplitude
converges, which a two-point comparison cannot settle.
...
\item \textbf{The \emph{amount} of steepening is not converged, and a short
      baseline can invert its sign.} The two grids agree on what the star loses
      --- $L_z$ at $0.076$ against $0.081\%$\,s$^{-1}$ --- and both steepen the
      rotation profile, but the $256^3$ onset is some $6$\,s later, so over any
      fixed window it has steepened less. Compared only out to $t = 26.5$\,s
      this reads as no steepening at all, and an earlier draft of this report
      concluded exactly that (Sec.~\ref{sec:latewindow}). Quote the direction
      and the rate after onset, not the total over a window.
\item \textbf{Beyond $40.5$\,s only $192^3$ exists.} The settled $1.50$\,s
...
$256^3$: running, $t = 55.8$\,s of a $60$\,s target; diagnostics processed to
$t = 40.5$\,s.

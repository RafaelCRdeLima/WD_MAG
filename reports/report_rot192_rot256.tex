...
$1.07$, while the mass stays within $0.3\%$ and the differential rotation is
not erased. The instability is non-axisymmetric, $m=1$, and grows in a few
...
$\Omega_{\mathrm{core}}/\Omega_{\mathrm{mean}}$ & $1.72$ & $1.70$ & \textbf{unchanged} \\
\midrule
...
and its differential rotation; the field is reduced by three orders of
magnitude in energy and ends with its two components equal in peak strength.}
\label{tab:end}
\end{table}
...
\paragraph{The differential rotation survives.}
$\Omega_{\mathrm{core}}/\Omega_{\mathrm{mean}}$ is $1.72$ over the first
snapshots and $1.70$ over the last: the shape of the rotation profile is the
same at $t = 60$\,s as at the start. The magnetic transport was real --- $L_z$
falls at $0.206\%$\,s$^{-1}$ while the toroidal field lives and $0.099\%$\,s$^{-1}$
after it is gone, a factor of $2.1$ --- but it never had the time or the
strength to erase the rotation that holds $2\,\Msun$ above the Chandrasekhar
mass. \emph{The star survives because it never lost what supports it.}

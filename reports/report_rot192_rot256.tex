...
$t = 12$\,s. The $192^3$ reaches $t = 60$\,s and the $256^3$
$t = 78$\,s, and over that baseline the two central dynamical claims converge:
\textbf{the profile steepens by $24.3\%$ against $25.4\%$}, each grid read to
the end of its own data, and \textbf{$L_z$ brakes at half the rate once the
field is gone --- $0.200$ against $0.214\%$\,s$^{-1}$ while it lives, $0.087$
against $0.094\%$\,s$^{-1}$ after --- so the identification of the transport as
magnetic holds at two resolutions}. Past $60$\,s the $256^3$ enters a
\emph{fourth} braking regime at $-0.037\%$\,s$^{-1}$, so the deceleration
continues rather than settling. What does not converge is the leftover field:
the residual magnetic energy is $25$ times larger at $256^3$ than at $192^3$,
and at $256^3$ it is still growing at $t = 78$\,s, $131\%$ above its minimum.
\textbf{One limitation governs how the rotation result must be read: the
magnetorotational instability is not resolved.} Measured from the
volume-typical vertical field, the quality factor $Q = \lambda_{\mathrm{MRI}}/
\Delta x$ peaks at $6.5$ near $t = 4.5$\,s and sits at $0.3$--$0.5$ over
$t = 40$--$78$\,s --- the window in which the steepening is measured. The
differential rotation therefore survives in a regime where the instability
that should erase it cannot exist on the grid (Fig.~\ref{fig:mri}).
\end{abstract}

%======================================================================
\section*{The limitation this report must be read against}
\label{sec:mri}

The central dynamical claim below is that the differential rotation is not
erased. It should be read knowing that the mechanism most likely to erase it is
absent from the calculation.

The magnetorotational instability grows on the wavelength
$\lambda_{\mathrm{MRI}} = 2\pi v_{\mathrm{A}z}/\Omega$, built from the field
component along the rotation axis. Representing it requires
$Q \equiv \lambda_{\mathrm{MRI}}/\Delta x \gtrsim 6$ for the linear mode and
$Q \gtrsim 15$--$20$ for converged turbulence. Figure~\ref{fig:mri} measures
$Q$ over the whole $256^3$ run from the \emph{volume-typical} vertical field,
$B_{\mathrm{pol,rms}} = \sqrt{8\pi\Epol/V}$, rather than from its peak; a peak
is not what sets the wavelength over the bulk of the star.

\begin{figure}[h]
\centering
\includegraphics[width=0.74\textwidth]{figures/mri_wavelength.pdf}
\caption{MRI quality factor at $256^3$. The band spans the three ways of
splitting the poloidal energy between $B_\varpi$ and $B_z$, from all of it
vertical to isotropic; the spread is a factor $1.73$, smaller than the effect
being measured. $Q$ peaks at $6.5$ at $t = 4.5$\,s --- during the $m=1$
disruption, when $\Epol$ is at its largest --- and decays to $\simeq 0.4$
thereafter. $Q \ge 6$ holds in $3\%$ of the $210$ samples under the central
assumption, in none under the isotropic one. $Q \ge 15$ is reached under no
assumption at any time. \emph{This figure alone uses the run extended to
$t = 78$\,s}, as is the rest of the report. Data:
\texttt{investigations/mri\_wavelength.py}.}
\label{fig:mri}
\end{figure}

Three consequences, stated here rather than in the caveats:

\begin{enumerate}\itemsep2pt
\item \textbf{Over the window in which the steepening is measured,
      $t = 40$--$78$\,s, $Q$ is $0.3$--$0.5$} --- twelve to twenty times short
      of the linear bar. ``The differential rotation survives'' and ``we have
      no way to erase it'' are not distinguished by this calculation.
\item \textbf{Converged MRI turbulence is out of reach on this mesh by
      construction}, not by accident of the run length. Reaching $Q = 15$ would
      need a resolution increase whose cost scales as $N^4$.
\item \textbf{The direction of the error is known.} Where the literature
      resolves the MRI in a differentially rotating magnetised remnant, it
      finds the inner rotation profile \emph{flattening} and angular momentum
      carried outwards \citep{miravet2025}. This report finds steepening. If
      the two are reconciled by the missing instability, the steepening is its
      absence rather than a property of the star.
\end{enumerate}

What is \emph{not} affected: the mass conservation, the stellar structure, the
$m=1$ disruption of the field, and the $L_z$ braking rates, none of which
depend on the MRI being present. Section~\ref{sec:caveats} lists the remaining
limitations.
...
%======================================================================
\section{Results: the field is destroyed}

The ordered field does not survive. It is disrupted by a non-axisymmetric
instability within a few Alfv\'en times, and what is left after thirty seconds
is a disordered residual three orders of magnitude weaker in energy. This
section states what happens; how much of it depends on resolution is deferred
to Sec.~\ref{sec:conv}.
...
%======================================================================
\section{Results: the star survives}

\paragraph{The mass does not move.} $0.02\%$ from start to end
(Table~\ref{tab:end}), $0.3\%$ in amplitude along the way, while the magnetic energy of the same star falls by a
...
%======================================================================
\section{Results: rotation and angular momentum}
\label{sec:rotation}

This is the property the whole configuration depends on: a $2\,\Msun$ white
dwarf exists only because the rotation is differential, so what the field does
to the rotation law decides whether the star has a future. Figures and numbers
here are $192^3$ over $60$\,s; Sec.~\ref{sec:latewindow} shows the $256^3$ run
doing the same thing, delayed and then faster, so the direction is a result and
...
supports it, and in the time it existed it steepened the profile rather than
flattening it.} Both halves of that sentence hold at $256^3$ as well
(Sec.~\ref{sec:latewindow}); what the finer grid does not reproduce is the
size of the steepening over a fixed window, because it starts later.
...
%======================================================================
\section{The end state at $192^3$}

Only the $192^3$ run has reached a steady late state; it was carried to
$t = 60$\,s and stopped changing well before that. The $256^3$ run stands at
$t = 40.5$\,s and is continuing.
...
$^{\ddagger}$These two rows measure how the star redistributes angular momentum
internally, and both converge over a long enough baseline: from $t = 12$ to
$58$\,s the $256^3$ run gives $-20.3\%$ and $-6.2\%$ against $-22.6\%$ and
$-10.6\%$ here. Over shorter windows it appears not to, because its onset is
some $6$\,s later (Sec.~\ref{sec:latewindow}).}
\label{tab:end}
\end{table}

%======================================================================
\section{Resolution: what is measured and what is mesh}
\label{sec:conv}

Everything above is physics as the two runs report it. This section asks which
of it would survive a finer grid. The answer splits three ways, and the split is
sharp enough to be useful: the \emph{star} converges, the \emph{energy} in the
field does not, and the \emph{rates} do not and move in opposite directions.

\subsection{The same instant, and the same window}
...
Table~\ref{tab:same} takes the two grids at one instant, $t \simeq 5.2$\,s, the
end of the growth phase and the most violent moment of the run. It is the
comparison that is easiest to make and the least trustworthy, and it is
...
\subsection{Past $12$\,s: three tests over a $46$\,s baseline}
\label{sec:latewindow}

The $256^3$ run reached $t = 78$\,s, so the two grids now overlap over almost
the whole evolution. Means of two pulsation periods at each end, so the
$\pm14\%$ breathing is averaged out rather than interpolated through.

\begin{table}[h]
\centering\small
\begin{tabular}{lrrl}
\toprule
$12$--$15$\,s $\to$ $55$--$58.3$\,s & $192^3$ & $256^3$ & verdict \\
\midrule
$L_z$ & $-2.6\%$ & $-2.9\%$ & converged \\
$\Omega_{\mathrm{out}}$ & $-15.3\%$ & $-14.9\%$ & converged ($3\%$) \\
$\Omega_{\mathrm{out}}/\Omega_{\mathrm{core}}$ & $-22.6\%$ & $-20.3\%$ & \textbf{converged ($10\%$)} \\
$\Omega_{\mathrm{core}}$ & $+9.4\%$ & $+6.7\%$ & same sign \\
$L_z^{\mathrm{outer}}/L_z^{\mathrm{inner}}$ & $-10.6\%$ & $-6.2\%$ & same sign \\
\bottomrule
\end{tabular}
\caption{The steepening of the differential rotation, over $46$\,s. The
amplitude that did not converge over shorter windows does converge here, to
$10\%$: the $256^3$ onset is late, not absent, and the lag washes out once the
baseline is long enough to contain it. The outer-shell spin-down agrees to
$3\%$.}
\label{tab:late}
\end{table}

The lag itself is visible only in segments, and it is why two earlier drafts of
this report reached two different wrong conclusions --- first that the
steepening was a coarse-grid artefact, then that its amplitude was
irreducibly resolution-dependent.

\begin{table}[h]
\centering\small
\begin{tabular}{lrr}
\toprule
change in $\Omega_{\mathrm{out}}/\Omega_{\mathrm{core}}$ & $192^3$ & $256^3$ \\
\midrule
...
\midrule
$t = 12 \to 26.5$\,s (a $14$\,s baseline) & $-8.2\%$ & $-1.0\%$ \\
$t = 12 \to 58.2$\,s (a $46$\,s baseline) & $-22.6\%$ & $-20.3\%$ \\
$t = 12 \to$ end of each run & $-24.3\%$ & $\mathbf{-25.4\%}$ \\
\bottomrule
\end{tabular}
\caption{The finer grid starts about $6$\,s later, then steepens faster. The
last three rows are the same comparison made over three baselines: at $14$\,s
the two grids appear to disagree by a factor of eight, at $46$\,s they agree to
$10\%$, and read to the end of each run ($60$ and $78$\,s) to $4.5\%$. Neither
number is wrong; the short one simply does not contain the onset, and the
agreement has improved every time the baseline has been extended. \emph{A convergence test over a window shorter than the lag between the
grids measures the lag, not the physics.}}
\label{tab:segments}
\end{table}

\paragraph{The braking rate halves, on both grids.} This is the strongest
result of the comparison. The claim that the angular-momentum transport is
magnetic rests on $L_z$ falling faster while the toroidal field lives than
after it is gone, and that was measured at one resolution. It is now measured
at two, in three successive regimes:

\begin{table}[h]
\centering\small
\begin{tabular}{lrrl}
\toprule
$\mathrm{d}\ln L_z/\mathrm{d}t$ & $192^3$ & $256^3$ & \\
\midrule
$t = 1.5$--$11.9$\,s \quad (field alive) & $-0.1996\%$/s & $-0.2144\%$/s & agree to $7\%$ \\
$t = 12$--$30$\,s \quad (field decaying) & $-0.0873\%$/s & $-0.0935\%$/s & agree to $7\%$ \\
$t = 30$--$58$\,s \quad (residual) & $-0.0446\%$/s & $-0.0523\%$/s & agree to $15\%$ \\
\bottomrule
\end{tabular}
\caption{Each regime brakes at about half the rate of the one before, and the
two grids agree on every one of them. With no explicit viscosity and no
explicit resistivity in the scheme, nothing else brakes anything --- so the
factor of two identifies the agent, and it does so at two resolutions.}
\label{tab:braking}
\end{table}

\paragraph{The residual field does not converge, and it is not decaying.} The
one place where the longer baseline makes things worse. At $192^3$ the magnetic
energy settles at $\simeq 3\times10^{46}$\,erg after $t \simeq 30$\,s. At
$256^3$ it settles at $7\times10^{47}$\,erg --- a factor of $25$ higher --- and
the peak field at $\simeq 6\times10^{12}$\,G against $\simeq 10^{12}$\,G.
Worse for interpretation, the finer grid's residual is not flat: fitted over
$t > 45$\,s it \emph{grows} at $+0.012$\,s$^{-1}$, with $\Etor$ and $\Epol$
both rising.

This is consistent with the $\mathrm{d}x^{2.4}$ scaling of the decay: less
numerical resistivity leaves more residual field. Taken at face value it says
the ``disordered residual'' reported at $192^3$ is a floor set by the mesh, not
by the star, and that the true residual is larger --- possibly much larger,
since a run that is still gaining magnetic energy at $t = 78$\,s has not
reached a final state at all. The obvious mechanism is available: the
differential rotation survives and is \emph{steepening}, so there is free shear
energy to wind residual poloidal field back into toroidal. Whether that is a
genuine dynamo or the last decade of a decay that the mesh cannot resolve is
not something two resolutions can settle.
...
\begin{enumerate}\itemsep2pt
\item \textbf{The MRI is not resolved.} Stated at the front of this report
      rather than here, because it governs how the rotation result must be
      read: $Q \simeq 0.4$ over the window in which the steepening is measured
      (Fig.~\ref{fig:mri}).
...
\item \textbf{A short baseline can invert the sign of a delayed effect.} The
      $256^3$ steepening begins about $6$\,s after the $192^3$ one. Measured to
      $t = 26.5$\,s the two grids appear to disagree by a factor of eight;
      measured to the end of each run they agree to $4.5\%$. Two earlier drafts of this
      report drew two different wrong conclusions from the short window
      (Sec.~\ref{sec:latewindow}). Any convergence test here needs a baseline
      longer than the lag between the grids, and the lag is not known in
      advance.
\item \textbf{The residual field is not converged and is not settled.} At
      $192^3$ the magnetic energy plateaus at $\simeq 3\times10^{46}$\,erg; at
      $256^3$ at $7\times10^{47}$\,erg, twenty-five times higher, and still
      \emph{rising} at $t = 78$\,s. Everything this report says about a
      ``disordered residual'' therefore describes a mesh-set floor rather than
      a state of the star, and the $192^3$ factor of $2170$ in $\Emag$ is an
      upper bound on the destruction. Whether the finer grid is showing a
      genuine shear dynamo --- the differential rotation is still there and
      still steepening, so the free energy exists --- or merely the last decade
      of a decay it cannot resolve, two resolutions cannot say.
...
$192^3$: complete, $t = 60$\,s. \quad $256^3$: complete at $t = 82.7$\,s, with
diagnostics processed to $78$\,s; every number here is measured over that.

The $256^3$ ended short of its $100$\,s target rather than reaching it:
restarting from the last checkpoint aborted after $37$\,s with
\texttt{too many subcycles}, deterministically, and the chain's own guard
stopped the sequence rather than resubmitting into the same wall.

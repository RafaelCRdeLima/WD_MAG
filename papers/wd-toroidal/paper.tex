...
\begin{document}
   ...
   {The dimensionless field strength $E_{\mathrm{mag}}/|W|$ is predicted
    ...
    velocity over which the sequence remains virially certified; and at a
    spin period of $25$\,s, at the fast end of observed white dwarf
    rotation, rotation is dynamically negligible for these
    configurations, contributing $T/|W| \simeq 10^{-4}$ and $0.03\%$ of
    the mass.}
   % conclusions
...
Table~\ref{tab:masses} and Fig.~\ref{fig:rhoc} show the measured values
at three central densities spanning two and a half decades. The
prediction was made before these runs and is confirmed by them:
$E_{\mathrm{tor}}/|W|$ varies by $0.9\%$ over the full range, and the
fractional mass gain by less than three percentage points. The lower
panel of Fig.~\ref{fig:rhoc} makes the point in the form
Eq.~\ref{eq:scaling} predicts it: the dimensionless field strength is
flat in $\rho_c$, and the two mass sequences in the upper panel are
therefore two nearly parallel curves separated by a constant factor,
rather than converging or diverging with density.

\begin{figure}
\centering
\includegraphics[width=\columnwidth]{figures/mass_vs_rhoc.pdf}
\caption{Equilibrium mass with and without a purely toroidal field
($m=1$, $K = 3\times10^{-3}$, no rotation) as a function of central
density (upper panel), and the dimensionless field strength of the
magnetized sequence (lower panel). The shaded region lies above the
inverse-beta-decay threshold for $\mu_e = 2$
($1.94\times10^{10}$\,g\,cm$^{-3}$, the conservative $^{16}$O value of
\citealt{boshkayev2013}), where the cold constant-$\mu_e$ equation of
state no longer describes a white dwarf; the $\rho_c = 10^{12}$
configuration is shown for reference only. The band in the lower panel
is the mean of the three measurements $\pm 1\%$.}
\label{fig:rhoc}
\end{figure}
...
Figure~\ref{fig:seq} shows how that mass is reached, along a continuous
sequence computed at a lower central density, $\rho_c = 10^9$\,g\,cm$^{-3}$,
where every configuration is comfortably inside the validity range of
the equation of state. The sequence is flat at first -- a field of
$10^{13}$\,G costs the star almost nothing -- and then steepens sharply,
crossing $2\,M_\odot$ at $B_{\varphi,\max} = 4.3\times10^{13}$\,G with
$\mathrm{VE} = 6.9\times10^{-4}$, still certified. Certification is
lost shortly afterwards, at $K = 4\times10^{-3}$: the last point of the
sequence is plotted open and is not a result. Note that
$B_{\varphi,\max}$ itself saturates along the sequence while the mass
does not, because the star expands as the field grows; it is the
integrated $E_{\mathrm{tor}}/|W|$, not the peak field, that tracks the
mass gain.

\begin{figure}
\centering
\includegraphics[width=\columnwidth]{figures/mass_vs_field.pdf}
\caption{Equilibrium mass along the toroidal sequence at
$\rho_c = 10^9$\,g\,cm$^{-3}$ ($m=1$, no rotation), obtained by
continuation from the field-free configuration. Filled symbols are
certified ($\mathrm{VE} < 10^{-3}$); the star marks the interpolated
$2\,M_\odot$ crossing.}
\label{fig:seq}
\end{figure}
...
\subsection{Where the field sits}
\label{sec:structure}

Figure~\ref{fig:cut} shows a meridional cut of the $2\,M_\odot$
configuration marked in Fig.~\ref{fig:seq}, solved directly at the
interpolated crossing value $K = 3.245\times10^{-3}$ rather than read off
the interpolation: it returns $M = 2.007\,M_\odot$ with
$\mathrm{VE} = 1.3\times10^{-4}$ and $f_{\mathrm{pol}} = 0.14$,
confirming the crossing of Fig.~\ref{fig:seq} to better than half a
percent.

The cut makes three features of this branch visible at once. The star is
prolate, $R_{\mathrm{pol}} = 5736$\,km against
$R_{\mathrm{eq}} = 5207$\,km, as a toroidal field must make it. It is
strongly centrally condensed, and anisotropically so: the
$\rho = 0.5\rho_c$ surface reaches $1.3\times10^8$\,cm along the axis but
only $0.27\times10^8$\,cm in the equatorial plane, an axis ratio near
$5$ in the core against $1.10$ at the surface. And the field, which by
Eq.~\ref{eq:fieldlaw} vanishes both on the rotation axis
($\varpi \to 0$) and where the density does, is confined to a tall,
narrow torus around the axis: $|B_\varphi|$ peaks at
$0.05\,R_{\mathrm{eq}}$ and has fallen to $9\%$ of that peak by
$0.5\,R_{\mathrm{eq}}$.

The mass gain of Sect.~\ref{sec:results} is therefore not produced by a
field threaded through the bulk of the star. It is produced by a compact,
intense torus in the inner few percent of the radius -- the region
holding roughly a quarter of the mass -- which inflates an envelope that
is itself almost field-free and reaches more than twice the field-free
radius. This is worth keeping in mind when comparing $B_{\varphi,\max}$
against an observationally inferred surface field: the two refer to
different regions of the star, and for this branch the surface field is
zero by construction (Sect.~\ref{sec:limitations}).

\begin{figure}
\centering
\includegraphics[width=\columnwidth]{figures/meridional_cut.pdf}
\caption{Meridional cut of the $M = 2.007\,M_\odot$ configuration
($\rho_c = 10^9$\,g\,cm$^{-3}$, $K = 3.245\times10^{-3}$, $m=1$, no
rotation), colored by the toroidal field strength. Panel (a) is the whole
star, with the stellar surface as the heavy contour; panel (b) is the
boxed core region. Thin white curves are the $\rho = 0.5\rho_c$ and
$\rho = 0.1\rho_c$ surfaces. The field is purely azimuthal: in this plane
it points out of the page on one side of the axis and into it on the
other, as marked at the field maximum. Both panels share the color
scale.}
\label{fig:cut}
\end{figure}
...
The expectation fails (Fig.~\ref{fig:rotation}). With the field present,
the equatorial mass-loss
...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{figures/rotation_termination.pdf}
\caption{Termination of the rotating sequence with and without a
toroidal field, at $\rho_c = 10^{12}$\,g\,cm$^{-3}$ on an identical
mesh. Upper panel: equatorial mass-loss ratio, which reaches unity at
breakup and is unphysical beyond it. Lower panel: virial error, with
the $10^{-3}$ acceptance threshold. With the field present both
diagnostics fail together in the shaded interval,
$\Omega_c \simeq 17$--$18$\,rad\,s$^{-1}$, well before the field-free
sequence shows any sign of terminating. The field-free point at
$\Omega_c = 26.5$\,rad\,s$^{-1}$, where the solver jumps to a spurious
branch, is omitted.}
\label{fig:rotation}
\end{figure}

\subsection{A rapidly rotating configuration}
\label{sec:fastrotator}

Section~\ref{sec:rotation} is a statement about the end of the rotating
sequence, at angular velocities of tens of rad\,s$^{-1}$. It says nothing
about whether a \emph{realistic} white dwarf rotation rate is compatible
with the field, which is the case that matters observationally, so we
computed one such configuration explicitly.

We take a spin period of $P = 25$\,s, at the fast end of what is observed
for white dwarfs, and impose it as rigid rotation
($\Omega_c = 0.251$\,rad\,s$^{-1}$) on the $\rho_c = 10^{10}$\,g\,cm$^{-3}$,
$K = 3\times10^{-3}$ configuration. Table~\ref{tab:rotating} reports it
against a non-rotating reference solved through the identical numerical
path, so that the difference between the two columns is the rotation term
and nothing else. Both satisfy every acceptance gate we apply to a
rotating configuration: $\mathrm{VE} < 10^{-3}$,
$f_{\mathrm{pol}} \le 0.2$, $T/|W| < 0.14$ (the secular
non-axisymmetric threshold of \citealt{ostriker1973}), and an equatorial
mass-loss ratio below unity.

\begin{table}
\caption{One certified rotating configuration
($\rho_c = 10^{10}$\,g\,cm$^{-3}$, $K = 3\times10^{-3}$, $m=1$, rigid
rotation), against a non-rotating reference computed through the same
path.}
\label{tab:rotating}
\centering
\begin{tabular}{l c c}
\hline\hline
 & no rotation & $P = 25$\,s \\
\hline
$M$ ($M_\odot$)                 & 2.0789 & 2.0796 \\
$R_{\mathrm{eq}}$ (km)          & 2671   & 2677 \\
$R_{\mathrm{pol}}$ (km)         & 2863   & 2863 \\
$R_{\mathrm{pol}}/R_{\mathrm{eq}}$ & 1.072 & 1.069 \\
$E_{\mathrm{tor}}/|W|$          & 0.1812 & 0.1812 \\
$T/|W|$                         & 0      & $9.9\times10^{-5}$ \\
mass-loss ratio                 & 0      & 0.005 \\
VE                              & $4.3\times10^{-5}$ & $4.3\times10^{-5}$ \\
\hline
\end{tabular}
\end{table}

Rotation at this rate is dynamically negligible for such a
configuration. It contributes $T/|W| \simeq 10^{-4}$ and raises the mass
by $0.03\%$, against the $47\%$ contributed by the field, even though the
equatorial velocity is $673$\,km\,s$^{-1}$ and the equatorial mass-loss
ratio is still only $0.005$ -- three orders of magnitude below breakup.
The deformation stays field-dominated: the star remains prolate, and
rotation reduces $R_{\mathrm{pol}}/R_{\mathrm{eq}}$ only from $1.072$ to
$1.069$, nowhere near flipping it to the oblate shape rotation alone
would produce. The toroidal field strength is unchanged to four
significant figures.

The two rotation results are therefore consistent, and together they
bound the question from both ends: a realistic fast white dwarf rotation
sits far below the regime where Sect.~\ref{sec:rotation} found the
combination degrading, and it also adds essentially nothing to the mass.
For this branch, the mass is carried by the field and not by the spin.
The non-rotating column above sits $0.3\%$ above the corresponding row of
Table~\ref{tab:masses}, computed on a coarser angular mesh; the two agree
to within that mesh difference, and the delta quoted here is taken
against the same-path reference rather than across tables.

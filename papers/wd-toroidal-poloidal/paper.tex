...
tight helix. The winding follows directly from the amplitude ratio of
Table~\ref{tab:config}: at $B_{\mathrm{t}}/B_{\mathrm{p}} = 26$ a field
line turns about twenty-six times around the axis for each circuit it
makes in the meridional plane. This is the twisted-torus geometry, with
the qualification that the twist here is extreme -- the poloidal circuit
is barely traced out at all before the line has wrapped the axis dozens
of times.
...
wireframe. Lines lie on nested flux surfaces and wind about twenty-six
times around the axis per meridional circuit, which is the amplitude
ratio $B_{\mathrm{t}}/B_{\mathrm{p}}$ of Table~\ref{tab:config} read as a
geometry.}
\label{fig:lines3d}
\end{figure}

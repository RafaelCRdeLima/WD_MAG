...
Figures~\ref{fig:meridional} and~\ref{fig:lines3d} show the field the
previous section quantifies. Both take a form dictated by the
axisymmetry: for an axisymmetric field $\mathbf{B}_{\mathrm{p}}$ is
perpendicular to $\nabla u$, so the poloidal field lines \emph{are} the
contours of the flux function, exactly, and are drawn as such rather than
integrated. The three-dimensional lines are integrated in cylindrical
...
stays between $10^{-4}$ and $3\times10^{-3}$.

The meridional cut shows the star is prolate (Table~\ref{tab:config}),
which is the deformation a toroidal field produces and the opposite of
rotation's. $B_\varphi$ is
concentrated well inside the surface, vanishing on the axis where
$\varpi \to 0$ and at the surface where the density does. The dashed
contour marks $\rho = 10^{-3}\rho_c$ and sits far inside the stellar
surface, which is the extended low-density envelope this configuration
carries: the field lives in the dense interior, not in the envelope it
inflates.

In three dimensions the same field is a set of nested tori wound by a
tight helix. Which tori exist is fixed by the flux at the stellar
...
region is the twisted torus proper, and it is where
Fig.~\ref{fig:lines3d} draws its lines. They are seeded by flux level
...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{figures/field_meridional.pdf}
\caption{Meridional cut of the adopted configuration. Colour is
$|B_\varphi|$, out of the plane. Black contours are the poloidal field
lines, which for an axisymmetric field are exactly the contours of the
flux function; orange arrows give the poloidal direction, drawn as unit
vectors because the poloidal amplitude spans orders of magnitude. The
heavy black curve is the stellar surface, prolate as a toroidal field
makes it; the dashed curve is $\rho = 10^{-3}\rho_c$.}

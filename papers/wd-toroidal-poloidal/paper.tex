...
wireframe. Each line is drawn for exactly one meridional circuit, and the
three are seeded at the flux levels labelled -- all inside the closed
region, $u > 0.775\,u_{\rm axis}$, so each lies on a torus contained in
the star. Winding rises outward across that region. The lines do not
close on themselves: the winding number is not an integer, so each
